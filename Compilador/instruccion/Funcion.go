...
func (p Function) Compilar(env *interfaces.Environment, tree *interfaces.Arbol, gen *interfaces.Generator) interface{} {
	...
	if p.Parametro != nil {
		...
		for _, s := range p.Parametro.ToArray() {
			...
			symbol := env.SearchSymbol(s.(ListExprefunc).Id)

			if symbol.Type != interfaces.NULL {

				excep := interfaces.NewException("Semantico", "Ya Existe ese Id "+s.(ListExprefunc).Id+" (function).", p.Row, p.Column)
			...
			result := interfaces.Value{Value: temp1, IsTemp: true, Type: s.(ListExprefunc).Type, TrueLabel: "", FalseLabel: ""}

			newTable.AddSymbol(s.(ListExprefunc).Id, result, s.(ListExprefunc).Type, true, newTable.Posicion, &newTable)
			tree.AddTableSymbol(*interfaces.NewTableSymbol(s.(ListExprefunc).Id, "Parametro", "Local", p.Row, p.Column, "--", fmt.Sprintf("%v", newTable.Posicion)))
			newTable.NewPos()
